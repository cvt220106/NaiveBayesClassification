 ...
 */
public class EvaluationJob extends Configured implements Tool {
    ...
    public static final Logger log = LoggerFactory.getLogger(EvaluationJob.class);

    public static void doEvaluation() {
        docTypeList = Const.TYPE_LIST.split("-");
        Path bayesResult = new Path(Const.GET_NAIVE_BAYES_CLASSIFICATION_JOB_OUTPUT_PATH + Const.HADOOP_DEFAULT_OUTPUT_FILE_NAME);
        ...
        try {
            ...
            File file = new File("F:\\opt\\module\\evaluation.txt");
            if (! file.exists()) {
                file.createNewFile();
            }
            ...
            for (String c : docTypeList) {
                ...
                System.out.println();
                fileWriter.write(c + " TP =" + TP + " FN= " + FN + " FP= " + FP + " TN= " + TN + "\n"
            ...
            System.out.println();
            fileWriter.write("Total TP= " + totalTP + "Total FN= " + totalFN + "Total FP= " + totalFP + "\n"
                             + "Total TN= " + totalTN + "\n" + "\n");
            ...
            fileWriter.write("微平均" + "\n"
                             + "Precision: " + precision + "\n"
                             + "Recall: " + recall + "\n"
                             + "F1: " + f1 + "\n" + "\n"
                             + "宏平均" + "\n"
                             + "Precision: " + totalPrecision / docTypeList.length + "\n"
                             + "Recall: " + totalRecall / docTypeList.length + "\n"
                             + "F1: " + totalF1 / docTypeList.length + "\n");
            fileWriter.close();
        } catch (Exception ex) {
